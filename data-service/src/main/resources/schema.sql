...
(
    ...
    CONSTRAINT UQ_DEPARTMENTS UNIQUE (department),
...
CREATE TABLE IF NOT EXISTS GEODESIC_REJECTION_PARAMETERS
(
    id                     BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    full_object            BOOLEAN,
    max_precipitation      INTEGER,
    max_neighboring_points INTEGER,
    max_diametric_point    INTEGER,
    measurement_error      FLOAT,
    CONSTRAINT pk_geodesicRejectionParameters PRIMARY KEY (id)
);
...
(
    ...
    rejection_parameter_id BIGINT,
    CONSTRAINT pk_surveyObject PRIMARY KEY (id),
    CONSTRAINT UQ_SURVEY_OBJECTS UNIQUE (object_type_id, building_id, stationary_number),
    ...
    CONSTRAINT FK_SURVEY_OBJECTS_ON_GEODESIC_REJECTION_PARAMETERS FOREIGN KEY (rejection_parameter_id)
        REFERENCES geodesic_rejection_parameters (id)
);

CREATE TABLE IF NOT EXISTS SURVEY_OBJECTS_REJECTION_PARAMETERS
(
    survey_object_id       BIGINT,
    rejection_parameter_id BIGINT,
    CONSTRAINT pk_survey_objects_of_geodesic_rejection_parameters PRIMARY KEY (survey_object_id, rejection_parameter_id)
);

CREATE TABLE IF NOT EXISTS ELEMENT_REJECTION_PARAMETERS
(
    id                BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    min               FLOAT,
    ...
    measurement_error FLOAT,
    CONSTRAINT pk_elementRejectionParameters PRIMARY KEY (id)
);
...
(
    ...
    CONSTRAINT FK_SURVEY_OBJECTS_ELEMENTS_ON_SURVEY_OBJECTS FOREIGN KEY (survey_object_id)
        REFERENCES survey_objects (id),
    CONSTRAINT FK_SURVEY_OBJECTS_ELEMENT_REJECTION_PARAMETERS FOREIGN KEY (rejection_parameter_id)
        REFERENCES element_rejection_parameters (id)
);
...
(
    ...
    CONSTRAINT UQ_REPORTING_DOCUMENTS UNIQUE (title, heading, document_type)
);
